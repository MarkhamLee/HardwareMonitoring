...
#!/usr/bin/env python
# simple script to generate unique IDs for each device, uses UUID4 as that ensures we get a truly
# unique ID and not one derived from the MAC address, with the associated security risks such an 
# approach can bring. 
# Putting this into its own script (for now) as so the various clients can use it + I'll probably
# put other utilities into this script, create an onboarding process, etc. 
...
class DeviceUtilities():
    ...
    # method for parsing the config file with connection data + the secrets file 
    @staticmethod
    ...
    def mqttClient(clientID, username, pwd, host, port):
        ...
        # this is so that the client will attempt to reconnect automatically/no need to add reconnect
        # logic. 
        client.loop_start()
